...
checkUrlContent() {
    ...
    set +e
    # Call the URL and get the HTTP status code
    HTTP_CONTENT=$(curl -s "$URL")
    set -e

    # Check if the HTTP status code is 200
    if [ "$HTTP_CONTENT" == $EXPECTED ]; then
        echo " ✔ $URL returned expected content."
        return 0
    else
        echo " ❌ $URL returned a status of $HTTP_CONTENT. Expected $EXPECTED."
        return 1
    fi
}
...
setup() {
  ...
  mongodbTestCaPem="$(cat .github/template/ssl/mongodb-cert.crt | base64)"
...
run_tests() {
  ...
  checkUrl "http://localhost:8085/example/test"
  checkUrl "http://localhost:8085/example/db"
  checkUrlContent "http://localhost:8085/example/db" '{"ok":{"foo":"bar"}}'
